...
const QRGenerator = () => {
  ...
  // ── FIX 3: Clear stale state when theaterId changes ──────
  useEffect(() => {
    setGeneratedSeats([]);
    setSelectedScreen("");
    setHallId("");
  ...
  // ── FIX 2: Load Seats with cinemaId cross-check ──────────
  const loadSeatsForHall = useCallback(
    async (hId) => {
      if (!hId) { setGeneratedSeats([]); return; }
      setProgress("Loading existing seats…");
      try {
        // ✅ cinemaId param prevents cross-cinema seat leakage
        const res = await fetch(
          `${API_BASE}/seat?hallId=${hId}&cinemaId=${theaterId}`,
          { headers: authHeaders() }
        );
        const data = await res.json();
        if (data.success && data.seats) {
          setGeneratedSeats(
            data.seats
              .map((s) => ({ _id: s._id, seatNumber: s.seatNumber, hallId: hId }))
              .sort((a, b) =>
  ...
  const mapScreenToHall = useCallback(
    async (screenNumber) => {
      ...
      try {
        ...
        const data = await res.json();
        if (!data.success) { setError("Failed to fetch halls."); return; }
        setHalls(data.halls || []);
  ...
  const generateSeats = async () => {
    setError("");
    if (!hallId)         { setError("Please select a screen first."); return; }
    if (!layout.length)  { setError("Please add at least one row."); return; }
    const seatList = [];
    for (const { row, count } of layout)
      for (let i = 1; i <= count; i++) seatList.push(`${row}${i}`);
    setGenerating(true);
    ...
    const saveSeat = async (seatNumber) => {
      try {
        ...
        const data = await res.json();
        return data.success && data.seat
          ? { _id: data.seat._id, seatNumber: data.seat.seatNumber, hallId }
          : null;
      } catch { return null; }
    };
    ...
    if (saved.length === 0)
      setError("No new seats saved. All may already exist for this screen.");
  };
  const clearLayout = () => {
    setLayout([]); setRowName(""); setSeatCount("");
    setError(""); setProgress(""); setProgressCount({ done: 0, total: 0 });
  };
  const resetAll = () => {
    clearLayout(); setGeneratedSeats([]); setSelectedScreen(""); setHallId("");
  };
  ...
  if (role !== "owner" && role !== "worker") return (
    <div className="qr-page"><div className="qr-error-card"><span className="qr-error-icon">🔒</span><h3>Access Denied</h3><p>Only owners and workers can access this page.</p></div></div>
  );
  if (!theaterData) return (
    <div className="qr-page"><div className="qr-error-card"><span className="qr-error-icon">🎭</span><h3>No Theater Found</h3><p>{error || "Please go back and select a theater."}</p><button className="btn-back" onClick={() => navigate(-1)}>← Go Back</button></div></div>
  );
      ...
      <div className="qr-layout">
        <aside className="qr-sidebar">
          <section className="qr-section">
            <label className="qr-label"><span className="qr-label-num">01</span>Select Screen</label>
            <select className="qr-select" value={selectedScreen} onChange={(e) => setSelectedScreen(e.target.value)} disabled={hallLoading || generating}>
              <option value="">— Choose a screen —</option>
              {Array.from({ length: screenCount }, (_, i) => i + 1).map((n) => (
                <option key={n} value={String(n)}>Screen {n}</option>
              ))}
            </select>
            {hallLoading && <p className="qr-hint qr-hint--loading"><span className="dot-pulse" /> Linking hall to screen…</p>}
            {hallId && !hallLoading && (
              <p className="qr-hint qr-hint--ok">
                ✓ Screen {selectedScreen} ready{generatedSeats.length > 0 && ` · ${generatedSeats.length} existing seats`}
              </p>
            )}
          </section>
          <section className="qr-section">
            <label className="qr-label"><span className="qr-label-num">02</span>Add Rows</label>
            <div className="qr-row-inputs">
              <input className="qr-input" placeholder="Row  (A, B, C…)" value={rowName} onChange={(e) => setRowName(e.target.value)} maxLength={3} disabled={!hallId || generating} onKeyDown={(e) => e.key === "Enter" && addRowToLayout()} />
              <input className="qr-input qr-input--sm" type="number" placeholder="# Seats" value={seatCount} onChange={(e) => setSeatCount(e.target.value)} min="1" max="50" disabled={!hallId || generating} onKeyDown={(e) => e.key === "Enter" && addRowToLayout()} />
              <button className="btn-add-row" onClick={addRowToLayout} disabled={!hallId || generating}>+ Add</button>
            </div>
            {layout.length > 0 && (
                ...
                {layout.map((r, i) => (
                  <div key={i} className="layout-row-item">
                    <div className="layout-row-label">
                      ...
                      <span className="row-seats">{r.count} seat{r.count > 1 ? "s" : ""} → {r.row}1 … {r.row}{r.count}</span>
                    </div>
                    <button className="btn-remove-row" onClick={() => removeRow(i)} title="Remove row">✕</button>
                  </div>
                ))}
                <p className="layout-total">{totalPending} new seat{totalPending !== 1 ? "s" : ""} to generate</p>
              </div>
            )}
          </section>
            ...
            <div className="qr-progress">
              ...
              {progressCount.total > 0 && (
                <div className="progress-bar-wrap">
                  <div className="progress-bar-fill" style={{ width: `${Math.round((progressCount.done / progressCount.total) * 100)}%` }} />
                  <span className="progress-label">{progressCount.done} / {progressCount.total}</span>
                </div>
              )}
            </div>
          )}
          <div className="qr-actions">
            <button className="btn-generate" onClick={generateSeats} disabled={generating || !hallId || layout.length === 0}>
              {generating ? <><span className="btn-spinner" /> Generating…</> : "⚡ Generate & Save Seats"}
            </button>
            <button className="btn-clear" onClick={clearLayout} disabled={generating}>Clear Layout</button>
            {(generatedSeats.length > 0 || selectedScreen) && (
              <button className="btn-reset" onClick={resetAll} disabled={generating}>↺ Reset Screen</button>
            )}
          </div>
        </aside>
        <main className="qr-main">
          {generatedSeats.length === 0 && !generating && (
            <div className="qr-empty">
              ...
              <p>{selectedScreen ? "No seats yet. Add rows and generate." : "Select a screen to get started."}</p>
            </div>
          )}
          {generatedSeats.length > 0 && (
            <>
              <div className="qr-grid-header">
                <h2>Screen {selectedScreen} — {generatedSeats.length} Seats</h2>
                <p className="qr-grid-subtitle">{theaterData.branchName || theaterData.name}</p>
              </div>
                  ...
                  return (
                    <div key={seat._id} className="qr-card">
                      <div className="qr-card-inner">
                        <QRCodeCanvas ref={(el) => (qrRefs.current[seat._id] = el)} value={url} size={110} bgColor="#ffffff" fgColor="#1a1a2e" level="M" includeMargin={false} />
                      </div>
                      <div className="qr-card-info">
                        <span className="qr-seat-number">{seat.seatNumber}</span>
                        <span className="qr-screen-label">Screen {selectedScreen}</span>
                      </div>
                    </div>
                  );
                })}
